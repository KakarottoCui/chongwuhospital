 ...
 * 回复评论 */
@Controller
public class HuifupinglunController extends BaseController
{
    @Autowired
    private HuifupinglunMapper dao;
    @Autowired
    private HuifupinglunService service;
     ...
     *
     */
    @RequestMapping("/huifupinglun_list")
    public String list()
    {
        ...
        Example example = new Example(Huifupinglun.class); //  创建一个扩展搜索类
        ...
        List<Huifupinglun> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
     ...
     * 预约人列表
     */
    @RequestMapping("/huifupinglun_list_yuyueren")
    public String listyuyueren()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Huifupinglun> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
        ...
        @RequestMapping("/huifupinglun_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/huifupinglun_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Huifupinglun mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/huifupingluninsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Huifupinglun post = new Huifupinglun();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
    ...
    * @return
    */
    @RequestMapping("/huifupinglunupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Huifupinglun post = new Huifupinglun();
        // 将前台表单数据填充到实体类
        if(!Request.get("bianhao").equals(""))
        post.setBianhao(Request.get("bianhao"));
                if(!Request.get("fuwuxiangmu").equals(""))
     ...
     *  后台详情
     */
    @RequestMapping("/huifupinglun_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Huifupinglun map = service.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    *  删除
    */
    @RequestMapping("/huifupinglun_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("huifupinglun").find(id);
